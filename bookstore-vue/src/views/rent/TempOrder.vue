<template>
  <v-container fluid>
    ...
    <v-row>
      <v-col>
        <v-card>
          <v-card-title>订单编号：{{ form.orderId }}<span class="ml-1">(请在10分钟内完成确认)</span></v-card-title>
          <v-card-text>
            <v-form ref="orderForm" v-model="valid" lazy-validation autocomplete="off">
              ...
              <v-row>
                <v-col>
                  <v-text-field :value="form.price" label="每周需支付金额(不足一周按一周算)" :rules="rules.priceRules"></v-text-field>
                </v-col>
              </v-row>
            </v-form>
          </v-card-text>
...
export default {
  ...
  methods: {
    ...
    generatedOrderId () {
      ...
      this.form.orderId = 'RBO' + snowflake.nextId().toString()
    },
    getItem () {
      // 获取所有商品，简略显示，并且计算总价
      this.axios.get(
    ...
    order () {
      ...
      if (valid) {
        this.axios.post(
          '/api/rentedBookOrders',
          {
            id: this.form.orderId,
            owner: this.$store.state.user._links.user.href,
